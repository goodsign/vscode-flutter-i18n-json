...
export class I18nCLIGenerator implements IDisposable, InsertActionProviderDelegate {
    ...
    async generateInitializeAsync(defaultLocale: string): Promise<void> {
        const config = <I18nConfig>{
            defaultLocale: defaultLocale,
            locales: [defaultLocale],
            localePath: I18nCLIGenerator.defaultI18nPath,
            generatedPath: I18nCLIGenerator.defaultGeneratedPath
        };
    ...
    async generateAddAsync(locale: string): Promise<void> {
    ...
    async generateRemoveAsync(pickedLocale: string): Promise<void> {
        const config = await this.readConfigFileAsync();
        if (!pickedLocale) {
            return;
        }
    ...
    private async generateDartFileAsync(config: I18nConfig): Promise<void> {
        ...
        dartContent += this.generateFunctions(I18nCLIGenerator.dart, "", false, undefined, functions);
        ...
        for (const locale of config.locales) {
            ...
            if (locale === config.defaultLocale) {
                dartContent += this.generateFunctions(
                    I18nCLIGenerator.dartLocale, locale, isRtl);
            } else {
                try {
                    const i18n = await this.readI18nFileAsync(locale, config);
                    const diff = this.diffFunctionTable(functions, i18n);
                    dartContent += this.generateFunctions(
                        I18nCLIGenerator.dartLocale, locale, isRtl, config.locales, diff, true);
                } catch (e) {
        ...
        dartContent += this.generateLocales(I18nCLIGenerator.dartGeneratedLocalizationsDelegate, config);
    ...
    async readConfigFileAsync(): Promise<I18nConfig> {
        const filename = this.fs.combinePath(this.workspaceFolder, I18nCLIGenerator.i18nConfigFile);
